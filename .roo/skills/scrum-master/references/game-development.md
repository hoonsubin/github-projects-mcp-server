...
A 2-year game ≈ 6–8 releases of 2–3 months. Each release groups sprints toward a milestone (vertical slice, alpha, content-complete, beta, gold). Release goals shift by stage: concept treatments → mechanics → asset volume → polish.
...
Cross-discipline is non-negotiable for feature teams. Discipline silos delay integration: programmers architect for design assumptions that change, artists make assets that won't fit the engine, designers spec features no one can build. Daily synchronization across disciplines is the point.

**Team size 7±2.** Coverage challenge: a level pre-prod team can need 11+ disciplines. Solutions: share specialists across teams (FX artist 25%-time on 4 teams), trade members between sprints, or accept one over-size team rather than splitting and adding dependencies.

**Self-organization includes membership.** Teams negotiate composition between releases. Mature studios let teams "self-organize people off." This is rare but it's the backstop that makes self-management real.
...
Each pipeline step is a column with a WIP limit. Cards (assets) flow left to right; pile-ups signal bottlenecks; empty columns signal starvation.
...
Goal: cycle time of each step ≤ takt time. If a step exceeds takt, parallelize or improve the pipeline. If a step's cycle time is less, that person picks up other work or rotates.
...
Mixed teams run the board with a swim lane: production work flows kanban-style; programmer work runs as a sprint inside the lane.
...
- **Designer as PO.** Often a fit (vision, player advocacy, cross-discipline communication). Pair with senior producer to cover gaps: ROI, schedule discipline, non-design constraints.

### QA

- **Embedded > pooled.** A tester on the team gives same-sprint feedback, helps with platform builds, tracks regressions. One tester per team until late in development.
- **QA in sprint planning.** QA helps define AC for each story before tasks are estimated. Stories aren't done until QA-verified.
- **Bug discovery curve flips.** Traditional projects find most bugs after alpha. Agile projects find them throughout; the post-alpha curve is shallow because nothing has been deferred.
- **Pre-alpha:** bugs go through the product backlog; PO prioritizes. **Post-alpha:** product backlog clears; the bug database becomes the work source. Daily triage with PO + QA lead.
- **Play-testing.** Recruit demographically diverse testers. Run sessions with developers watching. Expect to discover usability and pacing problems your team has gone blind to.

### Production (the role)

Producer responsibilities shift from "make sure everything gets done" to work teams can't do for themselves:
...
**Producer-as-SM caution:** producers from waterfall backgrounds revert to task assignment under stress. Coach them on facilitation/coaching stances, not status-tracking.

## 6. Scaling

### Scrum of Scrums (game variant)

Holds 2–3× weekly for 30–60 min. Reps from each team answer:
...
Align them. Same start/end dates across teams enables inter-team trades between sprints, integrated game build for review, and one PO time-window. Cost: PO is hammered during planning week. Worth it.

### Lookahead planning

Mid-sprint, teams + leads spend 1–2 hrs scanning the next 2–3 sprints' likely goals to surface specialist conflicts and resolve via priority shifts before sprint planning.
...
**Hardening sprints** before alpha and gold: timebox 1–2 sprints with no new feature work; backlog = bugs + polish + cert prep. Hardening that runs forever is a planning failure.

**Client-side Product Owner.** When a publisher funds the project, give them a single PO contact (the studio's lead PO) who is the only voice changing scope. Avoids the "every exec has a feature request" dysfunction.

**Stage gates as release reviews.** Each milestone review is a Sprint Review at scale: demo what's done, capture publisher feedback, refine the backlog.

## 9. Adoption

1. **Pilot one team.** Pick a feature with clear value and manageable scope. Mix volunteers across disciplines. SM should be the most coachable person available, not necessarily the most senior.
...
5. **Accept regression.** Crunch periods, late publisher changes, key person departures all surface old habits. Use retrospectives to name them.

**Cargo Cult Scrum** warning: studios that adopt the ceremonies without the principles (transparency, self-organization, working game every sprint) get the meeting overhead with none of the benefit and conclude "Scrum doesn't work for games."
